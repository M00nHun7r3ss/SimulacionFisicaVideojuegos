...
Particle::Particle(PxVec3 pos, PxVec3 vel) :
	...
	_m(1.0), _dump(0.999), _duration(5.0), _active(true), _size(1.0), _direction(PxVec3(1.0, 0.0, 0.0)), _orientation(PxQuat(PxIdentity))
{
...
Particle::Particle(PxVec3 pos, PxVec3 vel, Vector4 color) :
	...
	_m(1.0), _dump(0.999), _duration(5.0), _size(1.0), _direction(PxVec3(1.0, 0.0, 0.0)), _orientation(PxQuat(PxIdentity))
{
	//Creamos la forma con la geometria
	PxShape* sphere0 = CreateShape(PxSphereGeometry(_size));
...
Particle::Particle(PxVec3 pos, PxVec3 vel, Vector4 color, double masa) :
	...
	_m(masa), _dump(0.999), _size(2.0), _direction(PxVec3(1.0, 0.0, 0.0)), _orientation(PxQuat(PxIdentity))
{
	//Creamos la forma con la geometria
	PxShape* sphere0 = CreateShape(PxSphereGeometry(_size));
...
Particle::Particle(PxVec3 pos, PxVec3 vel, Vector4 color, PxVec3 acel, double masa, double dumping, double duration) :
	...
	_m(masa), _size(2.0), _dump(dumping), _direction(PxVec3(1.0, 0.0, 0.0)), _orientation(PxQuat(PxIdentity))
{
	// Creamos la forma con la geometria
	PxShape* sphere0 = CreateShape(PxSphereGeometry(_size));
...
Particle::Particle(PxVec3 pos, PxVec3 vel, Vector4 color, PxVec3 acel, double masa, double dumping, double duration, double size) :
	...
	_m(masa), _dump(dumping), _duration(duration), _size(size), _direction(PxVec3(1.0, 0.0, 0.0)), _orientation(PxQuat(PxIdentity))
{
	//Creamos la forma con la geometria
	PxShape* sphere0 = CreateShape(PxSphereGeometry(_size));
...
void Particle::rotateY(double angleRadians)
{
	PxQuat turn(angleRadians, PxVec3(0, 1, 0));
	_orientation = turn * _orientation;
}
